...
public class XToast {
    ...
    public static Toast custom(@NonNull Context context, @NonNull CharSequence message, Drawable icon,
        ...
        if (withIcon) {
            ...
            Utils.setBackground(toastIcon, XToast.Config.get().tintIcon ? Utils.tintIcon(icon, textColor) : icon);
        } else {
        ...
        if (XUI.getDefaultTypeface() == null) {
            toastTextView.setTypeface(XToast.Config.get().typeface);
        }
        if (XToast.Config.get().textSize != -1) {
            toastTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, XToast.Config.get().textSize);
        }
        if (XToast.Config.get().alpha != -1) {
            toastLayout.getBackground().setAlpha(XToast.Config.get().alpha);
        }
        currentToast.setView(toastLayout);

        if (!XToast.Config.get().allowQueue) {
            if (lastToast != null) {
                lastToast.cancel();
            }
            lastToast = currentToast;
        }
        if (XToast.Config.get().gravity != -1) {
            currentToast.setGravity(XToast.Config.get().gravity, XToast.Config.get().xOffset, XToast.Config.get().yOffset);
        }
        return currentToast;
    }

    public static class Config {
        private static volatile XToast.Config sInstance = null;
         ...
         * @return
         */
        public static XToast.Config get() {
            if (sInstance == null) {
                synchronized (XToast.Config.class) {
                    if (sInstance == null) {
                        sInstance = new XToast.Config();
                    }
                }
            }
            return sInstance;
        ...
        @CheckResult
        public XToast.Config setToastTypeface(@NonNull Typeface typeface) {
            this.typeface = typeface;
            return this;
        }

        @CheckResult
        public XToast.Config setTextSize(int sizeInSp) {
            this.textSize = sizeInSp;
            return this;
        }

        @CheckResult
        public XToast.Config tintIcon(boolean tintIcon) {
            this.tintIcon = tintIcon;
            return this;
        }

        public XToast.Config setAlpha(@IntRange(from = 0, to = 255) int alpha) {
            this.alpha = alpha;
            return this;
        }

        @CheckResult
        public XToast.Config allowQueue(boolean allowQueue) {
            this.allowQueue = allowQueue;
            return this;
        }

        public XToast.Config setGravity(int gravity) {
            this.gravity = gravity;
            return this;
        }

        public XToast.Config setGravity(int gravity, int xOffset, int yOffset) {
            this.gravity = gravity;
        ...
        public XToast.Config resetGravity() {
            gravity = -1;
            xOffset = 0;
            yOffset = 0;
            return this;
        ...
        public XToast.Config setXOffset(int xOffset) {
            this.xOffset = xOffset;
            return this;
        }

        public XToast.Config setYOffset(int yOffset) {
            this.yOffset = yOffset;
            return this;
        }
    }
